...
\newcommand{\usdrubstrike}{80}
\newcommand{\usdrubaxisspread}{8}
\newcommand{\usdrublow}{60}
\newcommand{\usdrubhigh}{110}
\newcommand{\highminusstrikepremium}{28}
\newcommand{\strikeminuslowpremium}{18}
\newcommand{\usdrubbarrier}{84}
...
\newcommand{\leftstrikeA}{79.5}
\newcommand{\leftstrikeB}{79.75}
\newcommand{\leftstrikeC}{79.95}
\newcommand{\rightstrikeA}{80.5}
\newcommand{\rightstrikeB}{80.25}
\newcommand{\rightstrikeC}{80.05}
...
\centering
  ...
  \addplot[Set1-A, very thick, dashed, samples=201] {(\x > \usdrubstrike)*(\x - \usdrubstrike) + 0.06};
\only<2->{
  \addplot[Set1-B, very thick, dashed, samples=201] {-(\x > \usdrubbarrier)*(\x - \usdrubbarrier) - 0.03};
}
\only<2>{
  \addplot[Set1-C, very thick, samples=201] {-(\x > \usdrubbarrier)*(\x - \usdrubbarrier) - 0.03 + (\x > \usdrubstrike)*(\x - \usdrubstrike) + 0.06};
}
\only<3->{
	\addplot[Set1-E, very thick, dashed, samples=201, domain=(\usdrubstrike-6):\usdrubbarrier] {-0.06};
	\addplot[Set1-E, very thick, dashed, samples=201, domain=\usdrubbarrier:(\usdrubstrike+10)] {-4};
}
\only<3>{
	\addplot[Set1-C, very thick, samples=201, domain=(\usdrubstrike-6):\usdrubbarrier] {-(\x > \usdrubbarrier)*(\x - \usdrubbarrier) - 0.03 + (\x > \usdrubstrike)*(\x - \usdrubstrike) + 0.06};
	\addplot[Set1-C, very thick, samples=201, domain=\usdrubbarrier:\usdrubstrike+10] {0.03};
}
...
\justify
1. Covered call: длинная позиция в индексе акций и проданный колл-опцион с высоким страйком.

\justify
2. Short straddle: проданный стрэддл с динамическим дельта-хеджированием (об этом позже).

\justify
3. Short OTM put: продажа опционов пут на индекс акций с низким страйком --- страховок от падения рынка.
...
\justify
Индекс S\&P\,500 сейчас находится на уровне \$6\,720 (17 марта 2026 г.). Пут-опцион со страйком \$4\,300, который истекает 30 апреля, стоит \$4.40 на \en{Chicago Mercantile Exchange (CME)}.

\justify
Если вы продадите 1\,000 таких опционов, то вы заработаете премию \$4\,400 практически без риска. В последний раз индекс S\&P\,500 падал на 35\% за 7 недель в 1929 году в разгар Великой депрессии.

\justify
Внимание: если Великая депрессия всё-таки повторится, и индекс упадёт до \$4\,000, то вы потеряете \$295\,600.
...
\justify
1. Продать одномесячный \en{at-the-money}\ пут-опцион на индекс \en{S\&P\,500}. Например, продать опцион со страйком $K=\$6\,720$, если текущий уровень индекса \$6\,720.
...
\addplot[color = Set1-C, mark = none, very thick, dashed]
   ...
   \draw[thick, color=black] (axis cs: 1985-01-01, 1) -- (axis cs: 2030-01-01, 1);
\end{axis}
\end{tikzpicture}
...
Геом. среднее & 9.7\% & 11.2\% & 3.0\% \\
Избыт. дох-ть & 6.4\% & 7.9\% & 0.0\% \\
Ст. откл. & 10.7\% & 15.7\% & 0.0\% \\
Отн. Шарпа & 0.60 & 0.50 & 0.00 \\
Мес. бета & 0.47 & 1.00 & 0.00
\end{tabular}

\small Период: 1987--2025. Все метрики, кроме беты, в годовом выражении.
