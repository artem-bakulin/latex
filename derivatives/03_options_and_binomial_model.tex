...
\justify
Европейский цифровой (\en{\alert{digital}}) опцион колл (пут) --- контракт, по которому 
...
через 3 месяца и номиналом 1\,000 рублей. Владелец получит 1\,000 рублей, если через 3 
...
\begin{frame}{Цифровые опционы: применение}
\justify
...
\begin{frame}{Цифровой опцион: репликация}
\justify
...
цифрового колла, $N$ --- число ванильных опционов в колл-спреде.
...
\begin{itemize}
\justifying
\item Колл выплачивает 1 единицу валюты, если цена базового актива $\ge K$ (*).
\item Пут выплачивает 1 единицу валюты, если цена базового актива $< K$.
\end{itemize}
...
\begin{frame}{Барьеры}
\justify
Выплата по опциону с барьером (\en{barrier}) зависит от того, примет ли цена базового 
актива определённое значение.

\justify
- \en{Knock-in (KI)}\ барьер: опцион возникает, только если цена достигает данного уровня.

- \en{Kock-out (KO)}\ барьер: опцион пропадает, если цена достигает данного уровня.

\justify
- Европейский барьер: наблюдается только в момент экспирации.

- Американский барьер: наблюдается всё время жизни опциона.

- Оконный барьер: наблюдается в определённый промежуток времени.

\justify
Обычно инвесторы добавляют в опционы барьеры, чтобы сделать их дешевле.
\end{frame}
...
ограничены снизу размером премии, а возможный выигрыш --- нет (для колл-опциона). Кто 
же тогда продаёт опционы, то есть соглашается на ограниченную прибыль в обмен на 
