...
\begin{tikzpicture}
	\node(s) at (0 ,0) {\$100};
	\node(su) at (3, 1) [anchor=west] {\$120, Payoff=\$20};
	\node(sd) at (3, -1) [anchor=west] {\$80, Payoff=\$0};

	\draw (s) [->] -- (su) node[above,midway] {90\%} ;
	\draw (s) [->] -- (sd) node[below,midway] {10\%};
	\draw (s) [dashed] -- (3, 0) node[anchor=west] {K=\$100};
\end{tikzpicture}
...
\begin{tikzpicture}
	\node(s) at (0 ,0) {$S_0$};
	\node(su) at (3, 1) [anchor=west] {$S_0u$, Payoff=$V_u$};
	\node(sd) at (3, -1) [anchor=west] {$S_0d$, Payoff=$V_d$};
...
\centering
\begin{tikzpicture}
	\node(s) at (0 ,0) {\only<1-2>{\$100} \only<9->{\$14.8}};
	\node(su) at (2, 0.5) [anchor=west] {\only<1-2>{\$120} \only<7->{\$25.8}};
	\node(sd) at (2, -0.5) [anchor=west] {\only<1-2>{\$80} \only<8->{\$3.8}};
	\node(suu) at (4, 1) [anchor=west] {\only<1-2>{\$144} \only<4->{\$44}};
	\node(sud) at (4, 0) [anchor=west] {\only<1-2>{\$96} \only<5->{\$7.6}};
	\node(sdd) at (4, -1) [anchor=west] {\only<1-2>{\$64} \only<6->{\$0}};
	\node(suuu) at (6, 1.5) [anchor=west] {\only<1-2>{\$172.8}  \only<3->{\$72.8}};
	\node(suud) at (6, 0.5) [anchor=west] {\only<1-2>{\$115.2}  \only<3->{\$15.2}};
	\node(sudd) at (6, -0.5) [anchor=west] {\only<1-2>{\$76.8}  \only<3->{\$0}};
	\node(sddd) at (6, -1.5) [anchor=west] {\only<1-2>{\$51.2} \only<3->{\$0}};
...
\begin{frame}{Биномиальная модель}
Важно: параметр $\Delta$ меняется в каждом узле.

\centering
\begin{tikzpicture}
	\node(s) at (0 ,0) {0.58};
	\node(su) at (2, 0.5) [anchor=west] {0.76};
	\node(sd) at (2, -0.5) [anchor=west] {0.24};
	\node(suu) at (4, 1) [anchor=west] {1.00};
	\node(sud) at (4, 0) [anchor=west] {0.40};
	\node(sdd) at (4, -1) [anchor=west] {0.00};
	\node(suuu) at (6, 1.5) [anchor=west] {\$72.8};
	\node(suud) at (6, 0.5) [anchor=west] {\$20};
	\node(sudd) at (6, -0.5) [anchor=west] {\$0};
	\node(sddd) at (6, -1.5) [anchor=west] {\$0};
...
\begin{tikzpicture}
	\node(s) at (0 ,0) {$S_0$};
	\node(su) at (1, 0.5) [anchor=west] {$S_0u$};
	\node(sd) at (1, -0.5) [anchor=west] {$S_0d$};
	\node(suu) at (3, 1) [anchor=west] {$S_0u^2$};
	\node(sud) at (3, 0) [anchor=west] {$S_0ud$};
	\node(sdd) at (3, -1) [anchor=west] {$S_0d^2$};
	\node(suuu) at (5, 1.5) [anchor=west] {$S_0u^3$, Payoff = $V_3=V(S_0u^3)$};
	\node(suud) at (5, 0.5) [anchor=west] {$S_0u^2d$, Payoff = $V_2=V(S_0u^2d)$};
	\node(sudd) at (5, -0.5) [anchor=west] {$S_0ud^2$, Payoff = $V_1=V(S_0ud^2)$};
	\node(sddd) at (5, -1.5) [anchor=west] {$S_0d^3$, Payoff = $V_0=V(S_0d^3)$};
...
\justify
<<Риск-нейтральная вероятность>>: $q=\dfrac{1+r\tau-d}{u-d}$

Цена опциона сейчас:
\begin{align*}
C =  \dfrac{q^3V_3 + 3q^2(1-q)V_2 + 3q(1-q)^2V_1 + (1-q)^3V_0}{(1+r\tau)^3} 
\end{align*}

Если представить, что $q$ --- вероятность движения цены вверх, то выше записано математическое ожидание.
\end{frame}
...
\vspace{\baselineskip}
Строгое доказательство (28 страниц) здесь:
...
\item Если биномиальное дерево состоит из $N$ шагов, то на каждом шаге цена акции умножается либо на $u=e^{\mu T/N +  \sigma\sqrt{T/N}}$ либо на $d=e^{\mu T/N -\sigma\sqrt{T/N}}$, то есть цена следует геометрическому броуновскому движению. Здесь $\sigma=const$ --- волатильность, $\mu$ --- тренд.
